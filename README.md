...
- [`android/`](android): All your `android`-specific implementations.
  - [`build.gradle`](android/build.gradle): The gradle build file. This contains four important pieces:
    ...
    3. Adds all Nitrogen files (`apply from: '.../NitroInput+autolinking.gradle'`)
  ...
  - [`CMakeLists.txt`](android/CMakeLists.txt): The CMake build file to build C++ code. This contains four important pieces:
    1. Creates a library called `NitroInput` (same as in `nitro.json`)
    2. Adds all Nitrogen files (`include(.../NitroInput+autolinking.cmake)`)
  ...
  - [`src/main/java/com/margelo/nitro/nitroinput/`](android/src/main/java/com/margelo/nitro/nitroinput/): All Kotlin implementations.
    - [`NitroInputPackage.java`](android/src/main/java/com/margelo/nitro/nitroinput/NitroInputPackage.java): The react-native package. You need this because the react-native CLI only adds libraries if they have a `*Package.java` file. In here, you can autolink all Kotlin HybridObjects.
...
- [`NitroInput.podspec`](NitroInput.podspec): The iOS podspec build file to build the iOS code. This contains three important pieces:
